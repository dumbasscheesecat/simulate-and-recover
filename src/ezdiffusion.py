...
class EZDiffusionModel:
    ...
    def simulate_estimated(self, R_obs, M_obs, V_obs):
        if R_obs == 1: #safeguard suggested by gpt so L does not return nan
            R_obs = 0.999
        elif R_obs < 0.1:
            R_obs = 0.001
    ...
    def recover_parameters(self, a, v, t, N): 
        ...
        for i in range(1000):
            ...
            interString = round(float(v - v_est), 6), round(float(a - a_est), 6), round(float(t - t_est), 6) #this and b^2 is suggested by gpt
