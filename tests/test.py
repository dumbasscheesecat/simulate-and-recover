...
from ezdiffusion import EZDiffusionModel  # Ensure this import works
...
class TestEZDiffusionModel(unittest.TestCase):
    ...
    def test_invalid_boundary_separation(self):
        with self.assertRaises(ValueError):
            self.model.add_condition(a=2.5, v=1, t=0.2)  

    def test_invalid_drift_rate(self):
        with self.assertRaises(ValueError):
            self.model.add_condition(a=1, v=-0.5, t=0.2)  

    def test_invalid_nondecision_time(self):
        with self.assertRaises(ValueError):
            self.model.add_condition(a=1, v=1, t=0.8)  

    def test_invalid_type_input(self):
        with self.assertRaises(ValueError):
            self.model.add_condition(a="string", v=1, t=0.2)  
    ...
    def test_unfitted_parameter_access(self):
        ...
        with self.assertRaises(AttributeError):
            _ = self.model.R_obs  # Should raise an error since it's not initialized yet
